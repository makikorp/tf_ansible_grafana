...
resource "null_resource" "grafana_install" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i aws_hosts --key-file /Users/ericmaki/.ssh/awsTerraTest playbooks/grafana.yml"
  }
}

output "grafana_access" {
  value = {for i in aws_instance.master[*] : i.tags.Name => "${i.public_ip}:3000"}
}